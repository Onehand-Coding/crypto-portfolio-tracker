...
class CryptoTrendAnalyzer:
    ...
    async def _fetch_data_source(self, symbol: str, period: str, interval: str) -> Optional[pd.DataFrame]:
        ...
        # 1. Try yfinance
        try:
            self.logger.info(f"Fetching {period} of {interval} data for {yf_ticker} from yfinance...")
            data = yf.download(tickers=yf_ticker, period=period, interval=interval, auto_adjust=True, progress=False)
            if not data.empty:
    ...
    async def fetch_crypto_data_async(self, symbol: str, period: str, interval: str = "1d") -> Optional[pd.DataFrame]:
    ...
    def _calculate_indicators(self, data: pd.DataFrame, settings: Dict[str, Any]) -> pd.DataFrame:
    ...
    def analyze_coin(self, data: pd.DataFrame, symbol: str, timeframe: str) -> TrendAnalysis:
        ...
        if data.empty or len(data) < 35:
            self.logger.warning(f"Insufficient data for {symbol} ({len(data)} periods). Cannot perform full analysis.")
            ...
            return TrendAnalysis(
                ...
                price_change_pct=0, active_conditions=[TrendCondition.INSUFFICIENT_DATA.value],
                rsi=50.0, support_level=0, resistance_level=0, ma_periods=None
            )
        ...
        price_change_pct = ((current_price / indicator_df['Close'].iloc[0]) - 1) * 100 if indicator_df['Close'].iloc[0] > 0 else 0
        ...
        if 'Volume' in indicator_df.columns and len(indicator_df) > 20 and pd.notna(indicator_df['Volume'].iloc[-1]):
             if latest.get('Volume', 0) > indicator_df['Volume'].rolling(window=20).mean().iloc[-1] * 2:
    ...
    def _evaluate_conditions(self, latest: pd.Series, previous: pd.Series, sma_short: float, sma_long: float, rsi: float) -> List[TrendCondition]:
        ...
        if pd.notna(macd_line) and pd.notna(macd_signal) and pd.notna(macd_line_prev) and pd.notna(macd_signal_prev):
    ...
    def _calculate_support_resistance(self, data: pd.DataFrame, window: int = 30) -> Tuple[float, float]:
    ...
    async def generate_report(self, timeframe: str) -> Optional[Dict[str, Any]]:
        ...
        report = {"timestamp": datetime.now().isoformat(), "timeframe": timeframe, "interval": interval}

        benchmark_symbol = "BTC-USD"
        if benchmark_symbol not in self.cryptocurrencies: self.cryptocurrencies.insert(0, benchmark_symbol)

        tasks = [self.fetch_crypto_data_async(coin, settings['period'], interval) for coin in self.cryptocurrencies]
        results = await asyncio.gather(*tasks)

        coin_data_map = {coin: data for coin, data in zip(self.cryptocurrencies, results)}
        valid_coin_data = {k: v for k, v in coin_data_map.items() if v is not None and not v.empty}

        if not valid_coin_data:
            self.logger.error("Failed to fetch data for any coins. Aborting report.")
            return None

        analyses = {coin: self.analyze_coin(data, coin, timeframe) for coin, data in valid_coin_data.items()}
        ...
        all_conditions = [cond for analysis in analyses.values() for cond in analysis.active_conditions]
        bullish_conditions = [TrendCondition.GOLDEN_CROSS.value, TrendCondition.PRICE_ABOVE_SMA200.value, TrendCondition.MACD_BULLISH_CROSS.value]
        bull_count = sum(1 for analysis in analyses.values() if any(c in analysis.active_conditions for c in bullish_conditions))

        report['market_summary'] = {
            "total_coins": len(analyses),
            "most_common_condition": max(set(all_conditions), key=all_conditions.count) if all_conditions else "None",
...
if __name__ == '__main__':
    async def main_test():
        try:
            ...
            analyzer = CryptoTrendAnalyzer(config=config_manager.config, binance_client=None)
            ...
            print("Could not import ConfigManager. Make sure this script is run from the 'src' directory.")
        except Exception as e:
