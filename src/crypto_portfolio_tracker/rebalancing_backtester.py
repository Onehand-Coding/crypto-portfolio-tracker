...
class RebalancingBacktester:
    ...
    def _pre_calculate_indicators(self, assets_to_calculate: List[str]):
        ...
        """
        self.logger.info(
            "Pre-calculating all technical indicators for the backtest period..."
        )
        ...
        for asset in assets_to_calculate:
            ...
            if close_col in self.data.columns:
                ...
                self.data[f"{asset}_SMA_L_long"] = ta.sma(
                    self.data[close_col], length=sma_long_len
                )
                sma_swing_len = swing_settings.get("sma_long_window", 30)
                self.data[f"{asset}_SMA_L_swing"] = ta.sma(
                    self.data[close_col], length=sma_swing_len
                )
                self.data[f"{asset}_RSI"] = ta.rsi(
                    self.data[close_col], length=rsi_period
                )
                sr_window = 30
                if high_col in self.data.columns and low_col in self.data.columns:
                    self.data[f"{asset}_Support"] = (
                        self.data[low_col].rolling(window=sr_window).min()
                    )
                    self.data[f"{asset}_Resistance"] = (
                        self.data[high_col].rolling(window=sr_window).max()
                    )
    ...
    def _fetch_and_prepare_data(self, symbols: list, period: str) -> bool:
        ...
        """
        cache_dir = (
            Path(self.config.get("cache", {}).get("path", "data/cache"))
            / "backtest_data"
        )
        ...
        self.logger.info(
            f"Fetching {period} of historical data for {len(symbols)} assets from yfinance..."
        )
        tickers = [f"{s.upper()}-USD" for s in symbols]
        try:
            self.data = yf.download(
                tickers, period=period, interval="1d", auto_adjust=True, timeout=60
            )
            if self.data.empty:
            ...
            self.logger.error(
                f"Critical error during yfinance download: {e}", exc_info=True
            )
            return False

        if "BTC_Close" not in self.data.columns or self.data["BTC_Close"].isna().all():
            self.logger.warning(
                "Failed to fetch benchmark (BTC) data. Buy & Hold comparison might be inaccurate."
            )
    ...
    def run(self, initial_capital: float, period: str, frequency: str = "monthly"):
        ...
        for asset in original_assets:
            close_col = f"{asset}_Close"
            if (
                close_col in self.data.columns
                and not self.data[close_col].dropna().empty
            ):
        ...
        if not assets_to_backtest:
            self.logger.error(
                "No valid historical data for any target assets for the selected period. Aborting backtest."
            )
            return

        original_target_allocation = self.config.get("target_allocation", {})
        adjusted_target_allocation = {
            asset: original_target_allocation[asset] for asset in assets_to_backtest
        }
        total_adj_alloc = sum(adjusted_target_allocation.values())

        if total_adj_alloc > 0:
            self.executed_allocation = {
                asset: alloc / total_adj_alloc
                for asset, alloc in adjusted_target_allocation.items()
            }
        else:
            self.executed_allocation = {
                asset: 1.0 / len(assets_to_backtest) for asset in assets_to_backtest
            }
    ...
    def run_simulation(
        ...
        for date in rebalance_dates:
            ...
            current_prices = {
                asset: self.data.loc[date, f"{asset}_Close"]
                for asset in target_allocation.keys()
            ...
            if not suggestions.empty:
                for index, suggestion_row in suggestions.iterrows():
                    ...
                    if price is None or price < min_price_threshold or np.isnan(price):
                        ...
                        self.logger.warning(
                            f"SIM: {date.date()}: Skipping {action} for {asset} due to invalid price ({price_str})."
                        )
                        continue
                    ...
                    if action == "BUY" and portfolio["USDT"] >= amount_usd:
                        ...
                        self.trade_log.append(
                            f"SIM: {date.date()}: BUY {quantity:.6f} {asset} @ ${price:,.2f}"
                        )
                    elif action == "SELL" and portfolio.get(asset, 0) >= quantity:
                        portfolio[asset] -= quantity
                        portfolio["USDT"] += amount_usd
                        self.trade_log.append(
                            f"SIM: {date.date()}: SELL {quantity:.6f} {asset} @ ${price:,.2f}"
                        )

        # Final portfolio value update
        last_date = self.data.index[-1]
        last_prices = {
            asset: self.data.loc[last_date, f"{asset}_Close"]
            for asset in target_allocation.keys()
            if f"{asset}_Close" in self.data.columns
        }
    ...
    def generate_report(self):
        ...
        results_df = (
            pd.DataFrame(self.portfolio_value_history)
            .set_index("date")
            .dropna(subset=["value"])
        )
        if results_df.empty:
        ...
        buy_hold_return = (
            (buy_hold_equity.iloc[-1] - self.initial_capital) / self.initial_capital
        ...
        sharpe_ratio = (
            (strategy_returns_pct.mean() * 252) / strategy_volatility
