...
def _print_wallet_summary(title: str, balances: List[Dict[str, Any]], balance_key: str, asset_key: str = 'asset'):
    """
...
def print_rebalance_suggestions(tracker: CryptoPortfolioTracker, suggestions_df: Optional[pd.DataFrame], available_usdt: Optional[float] = None):
...
async def view_trends(tracker: CryptoPortfolioTracker):
        ...
        """
        try:
            analyzer = CryptoTrendAnalyzer(config=tracker.config, binance_client=tracker.binance_client)
            print("✅ Trend Analyzer initialized with centralized config.")
        except Exception as e:
            tracker.logger.error(f"Failed to initialize CryptoTrendAnalyzer: {e}", exc_info=True)
        ...
        while True:
            ...
            if timeframe:
                ...
                try:
                    ...
                    tracker.logger.error(f"An error occurred during report generation for timeframe '{timeframe}': {e}", exc_info=True)
...
async def run_rebalancing_backtest(tracker: CryptoPortfolioTracker):
    ...
    try:
        ...
        initial_capital_str = await loop.run_in_executor(None, input, "Enter initial capital for simulation (default: 10000): ")
        initial_capital = float(initial_capital_str) if initial_capital_str else 10000.0

        period = ""
        while True:
            period_str = await loop.run_in_executor(None, input, "Enter backtest period (e.g., 2y, 3y, 5y - default: 3y): ")
...
async def run_manual_trade_session(tracker: CryptoPortfolioTracker):
    ...
    while trade_type not in ["BUY", "SELL"]:
        trade_type = await loop.run_in_executor(None, input, "Choose action [BUY / SELL]: ")
    ...
    # 3. Get Amount
    amount_str_input = await loop.run_in_executor(None, input, f"Enter amount to {trade_type} (e.g., '0.1 {symbol}'): ")
    ...
    if confirm == "EXECUTE":
        result = await tracker.execute_manual_trade_core(trade_type, symbol, trade_ticker, amount, is_quote_qty, is_live)
...
async def run_rebalance_and_execute(tracker: CryptoPortfolioTracker):
    ...
    print_rebalance_suggestions(tracker, suggestions_df, available_usdt=total_usdt_balance)
    ...
    while True:
        action = await loop.run_in_executor(None, input, "Type EXECUTE ALL or EXECUTE for one-by-one confirmation: ")
        ...
        if not action:
            ...
            tracker._sync_binance_client_time(tracker.binance_client, context="rebalancing")
            ...
            result = await tracker.execute_rebalancing_trades_core(suggestions_df, earn_balances, interactive=interactive_mode, auto_confirm=True)
            for msg in result.messages:
                print(msg)
            if result.success:
                print(f"✅ {result.data.get('trades_executed', 0)} trade(s) executed successfully!")
            else:
...
async def run_trading_strategy_backtest(tracker: CryptoPortfolioTracker):
    ...
    while strategy_to_run is None:
        try:
            choice_str = await loop.run_in_executor(None, input, f"Select the strategy to backtest (1-{len(strategy_list)}): ")
            ...
            if 0 <= choice < len(strategy_list):
                ...
                user_params = strategy_class.get_user_params() if hasattr(strategy_class, "get_user_params") else {}
    ...
    symbol_to_test = await loop.run_in_executor(None, input, "Enter the symbol to backtest (e.g., BTC-USD): ")

    try:
        initial_capital_str = await loop.run_in_executor(None, input, "Enter initial capital (default: 10000): ")
        initial_capital = float(initial_capital_str) if initial_capital_str else 10000.0
        period_str = await loop.run_in_executor(None, input, "Enter backtest period (e.g., '3y', '60d', default: '3y'): ")
        period = period_str if period_str else "3y"
    except ValueError:
        initial_capital = 10000.0; period = "3y"
...
async def run_live_strategy(tracker: CryptoPortfolioTracker):
        ...
        # 1. Select Account
        accounts = [{"name": "Main Account", "type": "main", **tracker.config.get("main_api_keys", {})}]
        ...
        for coin in target_coins:
            ...
            if data is None or data.empty:
                tracker.logger.warning(f"Could not fetch data for {yf_ticker}, cannot generate signal.")
                continue
...
def print_configuration(tracker: CryptoPortfolioTracker):
    ...
    if "sub_accounts" in safe_config:
        for account in safe_config["sub_accounts"]:
            if "binance_key" in account: account["binance_key"] = "********"
            if "binance_secret" in account: account["binance_secret"] = "********"
        ...
        try:
            ...
            match choice:
                ...
                case 5:
                    ...
                    sub_choice = await loop.run_in_executor(None, input, "Select option (1-2) or Enter to return: ")
                    if sub_choice == "1":
                ...
                case 7:
                    ...
                    sub_choice = await loop.run_in_executor(None, input, "Select option (1) or Enter to return: ")
                    if sub_choice == "1":
                        metrics = await tracker.calculate_portfolio_metrics()
