...
from .crypto_trend_analyzer import (
    CryptoTrendAnalyzer,
    TrendCondition,
    get_market_regime,
)
...
async def get_live_rebalance_suggestions(
    ...
    swing_report, long_term_report = await asyncio.gather(
        swing_report_task, long_term_report_task
    )
    ...
    if is_bear_market:
        analyzer.logger.warning(
            "BEARISH MARKET REGIME DETECTED. Rebalancing rules will apply."
        )
...
def get_backtest_rebalance_suggestions(
    ...
    for asset in target_assets:
        ...
        for generic_name, asset_specific_name in column_mapping.items():
            if asset_specific_name in full_historical_data_with_indicators:
                single_asset_df[generic_name] = full_historical_data_with_indicators[
                    asset_specific_name
                ]
        ...
        for col_name in full_historical_data_with_indicators.columns:
            if (
                col_name.startswith(f"{asset}_")
                and col_name not in column_mapping.values()
            ):
                single_asset_df[col_name] = full_historical_data_with_indicators[
                    col_name
                ]

        # Use the new historical analysis method with the correctly formatted DataFrame
        lt_analysis = analyzer.analyze_historical_row(
            single_asset_df, yf_symbol, "long_term", sim_date
        )
        sw_analysis = analyzer.analyze_historical_row(
            single_asset_df, yf_symbol, "swing", sim_date
        )
    ...
    portfolio_rows = [
        {"symbol": asset, "value_usd": quantity * latest_row.get(f"{asset}_Close", 0)}
        for asset, quantity in portfolio_state.items()
        if asset != "USDT"
    ]
    portfolio_rows.append(
        {"symbol": "USDT", "value_usd": portfolio_state.get("USDT", 0)}
    )
...
def generate_rebalancing_suggestions(
    ...
    target_allocation_normalized = {
        norm_map.get(k.upper(), k.upper()): v for k, v in target_allocation.items()
    }

    for symbol_simple, target_pct in target_allocation_normalized.items():
        yf_symbol = f"{symbol_simple}-USD"
        swing_analysis, long_term_analysis = (
            swing_report["coin_analyses"].get(yf_symbol),
            long_term_report["coin_analyses"].get(yf_symbol),
        )
        if not swing_analysis or not long_term_analysis:
            continue
        strategy_params = (
            rebalance_config.get("majors", {})
            if symbol_simple in major_assets
            else rebalance_config.get("alts", {})
        )
        drift_threshold_pct, current_value = (
            strategy_params.get("allocation_drift_threshold_pct", 5.0),
            portfolio_df[portfolio_df["symbol"] == symbol_simple]["value_usd"].sum(),
        )
        current_pct = (
        ...
        is_overweight, is_underweight = (
            absolute_drift_pct > drift_threshold_pct,
            absolute_drift_pct < -drift_threshold_pct,
        )
        long_term_conditions, is_oversold_long_term = (
            long_term_analysis.get("active_conditions", []),
            TrendCondition.RSI_OVERSOLD.value
            in long_term_analysis.get("active_conditions", []),
        )
        signal_strength, current_price = 1.0, swing_analysis.get("current_price", 0)
        support, resistance = (
            swing_analysis.get("support_level", 0),
            swing_analysis.get("resistance_level", 0),
        )
        if (
            is_underweight
            and support > 0
            and (current_price - support) / support < 0.10
        ):
            signal_strength = 1.5
        if (
            is_overweight
            and resistance > 0
            and (resistance - current_price) / resistance < 0.10
        ):
            signal_strength = 1.5
        ...
        if is_overweight:
            ...
            action_value_usd = (
                overweight_usd
                * strategy_params.get("sell_percentage_multiplier", 0.5)
                * signal_strength
            )
            ...
            if is_bear_market and suppress_buys_in_bear and not is_oversold_long_term:
                ...
                action_value_usd = (
                    underweight_usd
                    * strategy_params.get("buy_amount_multiplier", 1.0)
                    * signal_strength
                )
                coin_amount = (
                    action_value_usd / current_price if current_price > 0 else 0
                )
        ...
        suggestions_data.append(
            {
                ...
                "TA_Conditions": ", ".join(long_term_conditions) or "None",
                "Signal": signal,
