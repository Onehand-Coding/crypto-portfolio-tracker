...
class StrategyBacktester:
    ...
    def _reset_state(self, initial_capital: float):
        ...
        self.data = None
        self.logger.debug(
            f"Backtester state reset. Initial capital: ${self.initial_capital:,.2f}"
        )
    ...
    def _execute_trade(self, date, signal: str, price: float, reason: str, size: float):
        ...
        trade_fee_pct = (
            self.config.get("portfolio", {}).get("transaction_fee_percent", 0.1) / 100
        )
    ...
    async def run(
        ...
        self.data = await self.analyzer.fetch_crypto_data_async(
            symbol, period, interval
        )
        if self.data is None or self.data.empty:
            self.logger.error(
                f"Failed to fetch any data for {symbol}, cannot run backtest."
            )
            return

        self.logger.info(
            f"Starting simulation for {symbol} with strategy: {strategy.name}..."
        )
        ...
        for i in range(len(self.data)):
            ...
            signal, size, reason = await live_strategy_instance.generate_signal(
    ...
    def generate_report(self):
        ...
        print("\n" + "=" * 80)
        print(
            f"📈 BACKTEST PERFORMANCE REPORT: {self.symbol} ({self.data.index[0].year} - {self.data.index[-1].year})"
        )
