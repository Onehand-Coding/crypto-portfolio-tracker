...
class SymbolMapper:
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self.mappings_file_path = (
            Path(config.get("database", {}).get("path", "data/portfolio.db")).parent
        ...
        self._norm_map = {
            k.upper(): v.upper()
            for k, v in self.config.get("symbol_normalization_map", {}).items()
        }
    ...
    def __getitem__(self, symbol: str) -> str:
        ...
        raise KeyError(
            f"Discovery failed for symbol '{symbol}' (normalized: '{normalized_symbol}')"
        )
    ...
    def get(self, symbol: str, default: Any = None) -> Optional[str]:
        ...
        self.logger.debug(
            f"Symbol '{symbol}' not found via .get(). Triggering API discovery."
        )
        self.discover_mappings([normalized_symbol])
    ...
    @retry(
        ...
        ),  # Wait 4s, then 8s, 16s, etc.
        reraise=True,  # If all retries fail, raise the last exception
    )
    def _fetch_market_data_chunk_with_retry(
        self, chunk: List[str]
    ) -> List[Dict[str, Any]]:
        """
        ...
        try:
            ...
            if response.status_code == 429:
                self.logger.warning(
                    f"Rate limited (429) fetching market data. Tenacity will retry..."
                )
            ...
            raise NetworkOperationError(
                f"Failed to fetch market data from CoinGecko: {e}"
            )
        except Exception as e:
            self.logger.error(
                f"Unexpected error fetching market data: {e}", exc_info=True
            )
            raise NetworkOperationError(
                f"Failed to fetch market data from CoinGecko: {e}"
            )
    ...
    def _fetch_master_coins_list(self) -> List[Dict[str, str]]:
        if self._coins_list_cache and (
            time.time() - self._coins_list_last_updated < 86400
        ):
    ...
    def _score_candidate(self, candidate: Dict[str, Any]) -> tuple:
        is_derivative = any(
            keyword in candidate["id"] for keyword in self.derivative_keywords
        )
    ...
    def discover_mappings(self, symbols_to_find: List[str]):
        ...
        """
        self.logger.info(
            f"Attempting to discover mappings for {len(symbols_to_find)} symbols."
        )
        ...
        for symbol in unique_symbols:
            candidates = [
                c for c in coins_list if c.get("symbol", "").upper() == symbol
            ]
            if not candidates:
            ...
            if len(candidates) == 1:
                ...
                self.logger.info(
                    f"AUTOMAP: Found unique match for '{symbol}': {candidates[0]['id']}"
                )
            else:
        ...
        if ambiguous_symbols:
            self.logger.warning(
                f"Found {len(ambiguous_symbols)} ambiguous symbols. Resolving in chunks..."
            )
            market_data_map = {}
            ...
            for i in range(0, len(all_ids_list), chunk_size):
                ...
                self.logger.info(
                    f"Fetching market data for chunk {i // chunk_size + 1}..."
                )
                try:
                    ...
                    self.logger.error(
                        f"AUTOMAP: API error on chunk {i // chunk_size + 1} after all retries: {e}."
                    )
