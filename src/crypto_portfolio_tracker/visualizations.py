...
class Visualizer:
    ...
    def create_portfolio_allocation_pie(
        ...
        if holdings_df.empty or "value_usd" not in holdings_df.columns:
            ...
            return
        fig, ax = plt.subplots(
            figsize=(self.figure_size[0] / 2, self.figure_size[1] / 2)
        )
        data = holdings_df.set_index("symbol")["value_usd"]
        wedges, texts, autotexts = ax.pie(
            data, autopct="%1.1f%%", startangle=90, pctdistance=0.85
        )
        ax.set_title("Portfolio Allocation (by Value)", fontsize=16, pad=20)
    ...
    def generate_all_charts(
        ...
        self.create_portfolio_allocation_pie(holdings_df, metrics)
        self.create_allocation_comparison_bar(holdings_df, target_allocation)
        self.create_pl_by_asset_bar(holdings_df)
        self.create_portfolio_value_history(snapshots_df)
