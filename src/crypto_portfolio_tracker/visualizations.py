...
class Visualizer:
    ...
    def create_portfolio_allocation_pie(self, holdings_df: pd.DataFrame, metrics: Dict[str, Any]):
        ...
        if holdings_df.empty or 'value_usd' not in holdings_df.columns: self.logger.warning("Cannot create allocation pie: No holdings data."); return
    ...
    def create_allocation_comparison_bar(self, holdings_df: pd.DataFrame, target_allocation: Dict[str, float]):
        ...
        if holdings_df.empty or not target_allocation: self.logger.warning("Cannot create allocation comparison: No holdings or target."); return
    ...
    def create_pl_by_asset_bar(self, holdings_df: pd.DataFrame):
        ...
        if holdings_df.empty or 'unrealized_pl_usd' not in holdings_df.columns: self.logger.warning("Cannot create P/L bar: No P/L data."); return
    ...
    def create_portfolio_value_history(self, snapshots_df: pd.DataFrame):
        ...
        if snapshots_df.empty: self.logger.warning("Cannot create value history: No snapshot data."); return
