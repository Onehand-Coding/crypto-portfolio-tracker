...
class Visualizer:
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self.export_path = Path(config.get("exports", {}).get("path", "data/exports/")) / "charts"
    ...
    self, holdings_df: pd.DataFrame, metrics: Dict[str, Any], 
    save_to_disk: bool = True, return_bytes: bool = False
) -> Optional[bytes]:
        ...
        significant_holdings = holdings_df[holdings_df["value_usd"] >= min_threshold].copy()
        ...
        if not small_holdings.empty:
            ...
            significant_holdings = pd.concat([significant_holdings, others_row], ignore_index=True)
        ...
        colors = [self.colors['primary'], self.colors['secondary'], self.colors['success'], 
                self.colors['danger'], self.colors['purple'], self.colors['brown'], 
                self.colors['pink'], self.colors['gray'], self.colors['olive'], self.colors['cyan']]
        ...
        wedges, texts, autotexts = ax.pie(
            ...
            textprops={'fontsize': 16, 'weight': 'bold', 'color': 'white'},  # Larger, bold white text
            labeldistance=1.1  # Move labels further from pie
        )
        
        # Set title with larger font
        ax.set_title("Portfolio Allocation (by Value)", fontsize=24, pad=30, weight='bold')
        ...
        def create_interactive_allocation_pie(self, holdings_df: pd.DataFrame) -> go.Figure:
            ...
            for symbol, value in zip(data.index, data.values):
                ...
                hover_text.append(f"{symbol}<br>Value: ${value:,.0f}<br>Percentage: {pct:.1f}%")
    ...
    def create_pl_by_asset_bar(
        ...
        colors = [self.colors['danger'] if x < 0 else self.colors['success'] for x in data]
    ...
    def create_portfolio_value_history(
        ...
        snapshots_df.plot(kind="line", y="total_value_usd", ax=ax, marker="o", 
    ...
    save_to_disk: bool = True, return_bytes: bool = False
        ...
        bars = comparison_df.plot(kind="bar", ax=ax, width=0.7, 
                                color=[self.colors['primary'], self.colors['secondary']],
                                edgecolor='white', linewidth=1.5)
        
        # Enhanced title and labels
        ax.set_title("Current vs. Target Portfolio Allocation", fontsize=18, pad=25, weight='bold')
        ...
        # Enhanced legend
        legend = ax.legend(title="Allocation Type", fontsize=12, title_fontsize=13, 
    ...
    def create_interactive_pl_by_asset(self, holdings_df: pd.DataFrame) -> go.Figure:
        ...
        hover_text = [f"{symbol}<br>P/L: ${value:,.0f}" for symbol, value in zip(data.index, data.values)]
    ...
    def create_interactive_value_history(self, snapshots_df: pd.DataFrame) -> go.Figure:
        ...
        # Create custom hover text
        hover_text = [f"Date: {date}<br>Value: ${value:,.0f}" 
    ...
    def generate_all_charts(
        ...
        self.create_allocation_comparison_bar(holdings_df, target_allocation, save_to_disk)
    ...
    def get_chart_bytes(self, chart_type: str, data: Dict[str, Any], 
