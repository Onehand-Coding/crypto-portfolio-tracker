...
class PriceEnricher:
    ...
    def __init__(
        ...
        self.stablecoin_symbols = config.get("portfolio", {}).get(
            "stablecoin_symbols", []
        )
        ...
        if self.coingecko_config.get("api_key"):
            ...
            self.logger.info(
                "PriceEnricher initialized without a CoinGecko API Key (using public access)."
            )
        self.logger.debug(
            f"Concurrency limit set to {self.coingecko_semaphore._value} for price fetching."
        )
    ...
    def _get_historical_fiat_exchange_rate(
        self, date: datetime.datetime, from_currency: str, to_currency: str
    ) -> float:
        ...
        try:
            ...
            self.logger.warning(
                f"Could not fetch yfinance rate for {ticker} on {date_str}: {e}"
            )
    ...
    async def _fetch_price_for_date(
        self, client: httpx.AsyncClient, coin_id: str, date_str: str
    ):
        cache_key = f"{coin_id}_{date_str}"
        if (
            self.price_cache.get(cache_key) is not None
            or self.disk_cache.get(cache_key) is not None
        ):
        ...
        async with self.coingecko_semaphore:
            await asyncio.sleep(
                self.coingecko_config.get("request_delay_ms", 1500) / 1000.0
            )
            base_url = self.coingecko_config.get(
            ...
            try:
                ...
                if price is not None:
                    self.logger.debug(
                        f"API SUCCESS: Fetched price for {coin_id} on {date_str}: ${price}"
                    )
                ...
                self.logger.error(
                    f"An unexpected error in _fetch_price_for_date for {coin_id}: {e}"
                )
                self.price_cache[cache_key] = None
    ...
    async def _batch_fetch_prices(self, price_requests: Set[Tuple[str, str]]):
        ...
        self.logger.debug(
            f"Starting controlled batch fetch for {len(price_requests)} unique prices..."
        )

        missing_requests = {
            (coin_id, date_str)
            for coin_id, date_str in price_requests
            if self.price_cache.get(f"{coin_id}_{date_str}") is None
            and self.disk_cache.get(f"{coin_id}_{date_str}") is None
        }

        if not missing_requests:
            self.logger.debug(
                "All requested prices were already in the cache. Nothing to fetch."
            )
            return
        ...
        async with httpx.AsyncClient() as client:
            tasks = [
                self._fetch_price_for_date(client, coin_id, date_str)
        ...
        for coin_id, date_str in missing_requests:
            cache_key = f"{coin_id}_{date_str}"
            if (
                self.price_cache.get(cache_key) is None
                and self.disk_cache.get(cache_key) is None
            ):
                all_successful = False
                self.logger.warning(
                    f"Verification failed: Price for {coin_id} on {date_str} is still missing after batch."
                )
                break

        if not all_successful:
            self.logger.error(
                "One or more price fetches failed in the batch. Raising error to trigger retry."
            )
            raise NetworkOperationError(
                "Batch fetch incomplete, could not retrieve all requested prices from CoinGecko."
            )
        ...
        async with httpx.AsyncClient() as client:
            try:
                response = await client.get(
                    url, params=params, timeout=self.coingecko_config.get("timeout", 30)
                )
                response.raise_for_status()
                ...
                self.logger.warning(
                    f"Rate limited fetching current prices: {e}. Retrying..."
                )
                raise
            except Exception as e:
                self.logger.error(
                    f"An unexpected error occurred while fetching current prices: {e}"
                )
                raise NetworkOperationError(
                    f"Failed to fetch current prices from CoinGecko: {e}"
                )

        # If we reach here, all retries failed (shouldn't happen due to tenacity, but for safety)
        raise NetworkOperationError(
            "Failed to fetch current prices from CoinGecko after retries."
        )
    ...
    async def enrich_transactions(
        ...
        self.logger.debug(
            f"Passing {len(transactions)} raw transactions to the PriceEnricher."
        )
        price_requests: Set[Tuple[str, str]] = set()
        for tx in transactions:
            if not tx.get("timestamp") or not isinstance(
                tx.get("timestamp"), datetime.datetime
            ):
                continue
            ...
            tx_type = tx["tx_type"]
            if tx_type == "TRADE":
                symbols_to_price.extend(
                    [raw.get("quote_asset"), raw.get("fee_currency")]
                )
            ...
            for symbol in symbols_to_price:
                if symbol and symbol not in self.stablecoin_symbols:
                    coin_id = self.symbol_mappings.get(symbol)
                    if coin_id:
                        price_requests.add((coin_id, date_str))
        ...
        for tx in transactions:
            ...
            if tx["tx_type"] == "CONVERT":
                    ...
                    enriched_transactions.append(
                        {
                            ...
                            "timestamp": ts,
                            "type": "SELL",
                            "quantity": raw["from_quantity"],
                            "price_usd": price_usd,
                            "fee_quantity": 0,
                            "fee_currency": None,
                            "fee_usd": 0,
                            "source": tx["source"],
                            "transaction_hash": f"convert_sell_{tx['transaction_hash']}",
                            "notes": f"Converted to {raw['to_quantity']} {to_asset}",
                        }
                    )
                    ...
                    enriched_transactions.append(
                        {
                            ...
                            "timestamp": ts,
                            "type": "BUY",
                            "quantity": raw["to_quantity"],
                            "price_usd": price_usd,
                            "fee_quantity": 0,
                            "fee_currency": None,
                            "fee_usd": 0,
                            "source": tx["source"],
                            "transaction_hash": f"convert_buy_{tx['transaction_hash']}",
                            "notes": f"Converted from {raw['from_quantity']} {from_asset}",
                        }
                    )
                ...
                price_usd = (
                    raw["price"] * quote_price_usd if quote_price_usd is not None else 0
                )
                fee_price_usd = self._get_price_from_cache(raw["fee_currency"], ts)
                fee_usd = (
                    raw["fee_quantity"] * fee_price_usd
                    if fee_price_usd is not None
                    else 0
                )
                enriched_transactions.append(
                    {
                        "symbol": raw["base_asset"],
                        "timestamp": ts,
                        "type": "BUY" if raw["is_buyer"] else "SELL",
                        "quantity": raw["quantity"],
                        "price_usd": price_usd,
                        "fee_quantity": raw["fee_quantity"],
                        "fee_currency": raw["fee_currency"],
                        "fee_usd": fee_usd,
                        "source": tx["source"],
                        "transaction_hash": tx["transaction_hash"],
                        "notes": f"Pair: {raw['base_asset']}/{raw['quote_asset']}",
                    }
                )
                if raw["quote_asset"] in self.target_assets_for_sync:
                    enriched_transactions.append(
                        {
                            "symbol": raw["quote_asset"],
                            "timestamp": ts,
                            "type": "SELL" if raw["is_buyer"] else "BUY",
                            "quantity": raw["quantity"] * raw["price"],
                            "price_usd": quote_price_usd,
                            "fee_quantity": 0,
                            "fee_currency": None,
                            "fee_usd": 0,
                            "source": "Binance Synthetic",
                            "transaction_hash": f"synth_{tx['transaction_hash']}",
                ...
                price_usd = self._get_price_from_cache(raw["symbol"], ts)
                fee_price_usd = self._get_price_from_cache(
                    raw.get("fee_currency", raw["symbol"]), ts
                )
                fee_usd = (
                    raw.get("fee_quantity", 0.0) * fee_price_usd
                    if fee_price_usd is not None
                    else 0
                )
                tx_type_final = (
                    "WITHDRAWAL" if tx["tx_type"] == "WITHDRAWAL" else "SELL"
                )
                enriched_transactions.append(
                    {
                        "symbol": raw["symbol"],
                        ...
                        "quantity": raw["quantity"],
                        "price_usd": price_usd,
                        "fee_quantity": raw.get("fee_quantity", 0.0),
