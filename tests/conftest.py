...
def mock_config_manager():
    ...
    config.symbol_mapper.get_symbol_mapping.return_value = {"BTC": "bitcoin", "ETH": "ethereum"}
...
def db_manager(test_db_path):
    ...
    return DatabaseManager(
        ...
        backup_dir=test_db_path.parent / "backups",
        cleanup_days=7
    )
...
def tracker(mock_config_manager):
    ...
    with patch('crypto_portfolio_tracker.portfolio_tracker.BinanceFetcher') as mock_fetcher_class:
        mock_fetcher = Mock()
        ...
        with patch('crypto_portfolio_tracker.portfolio_tracker.PriceEnricher') as mock_enricher_class:
            mock_enricher = Mock()
            mock_enricher.get_current_prices = AsyncMock(return_value={"BTC": 50000, "ETH": 3000})
            mock_enricher_class.return_value = mock_enricher
            
            with patch('crypto_portfolio_tracker.portfolio_tracker.CryptoTrendAnalyzer') as mock_analyzer_class:
                mock_analyzer = Mock()
